        ...
        def self.search(search, compare, year, rain_fall_type)
          if search == 'All'
            ...
            if year == "All"
              ...
              where('Districts = ? OR Districts = ?', search, 'Total').where('year = ?', year).order(:id)
            end
         ...
         if _year == "All"
          ...
          if rain_fall_type == "Number_of_Document"
            b.each do |x|
              ...
              grouped[x[:Districts]][x[:Year]] = x[:Number_of_Document]
            end
          elsif rain_fall_type == "Registration_Fees"
            b.each do |x|
              grouped[x[:Districts]] ||= {}
              grouped[x[:Districts]][:Districts] = x[:Districts]
              grouped[x[:Districts]][x[:Year]] = x[:Registration_Fees]
            end
          elsif rain_fall_type == "Stamp_Fee"
            b.each do |x|
              grouped[x[:Districts]] ||= {}
              grouped[x[:Districts]][:Districts] = x[:Districts]
              grouped[x[:Districts]][x[:Year]] = x[:Stamp_Fee]
            end
          elsif rain_fall_type == "Total_Receipt"
            b.each do |x|
              grouped[x[:Districts]] ||= {}
              grouped[x[:Districts]][:Districts] = x[:Districts]
              grouped[x[:Districts]][x[:Year]] = x[:Total_Receipt]
            end
          elsif rain_fall_type == "Target"
            b.each do |x|
              grouped[x[:Districts]] ||= {}
              grouped[x[:Districts]][:Districts] = x[:Districts]
              grouped[x[:Districts]][x[:Year]] = x[:Target]
            end
          else
          end
          ...
          if rain_fall_type == 'All'
            ...
            if compare == 'District vs Total'
              ...
              if _year == "All"
                ...
                if search == "All"
                  ...
                  hash_data  = grouped_data.map{ |vegetable, values| 
                    dataset = vegetable.to_s.gsub("_"," ")
                    {
                      ...
                      name:dataset,
                      showInLegend: true,
                      dataPoints: values.reject { |x| x['Districts'] == 'Total' }.map { |value|
                        { y: value[rain_fall_type], label: value["Year"] }
                      }
                    }
                 }
                ...
                if views != "column" && views!="line"
                  dataset = rain_fall_type.tr('_', ' ')
                  hash_data =  b.reject { |x| x['Districts'] == 'Total' }.map do |el|
                    {
                      type:views,
                      toolTipContent: "{label}<br/>{name}, <strong>{y}</strong>",
                      name:"#{el["Districts"]}",
