class AgroBasedIndustries3 < ApplicationRecord
  ...
  def self.search(search, compare, year, rain_fall_type)
    if search == 'All'
      ...
      if year == "All"
        where('Union_Project = ? OR Union_Project = ?', search, compare).order(:id)
      else
        where('Union_Project = ? OR Union_Project = ?', search, compare).where('year = ?', year).order(:id)
      end
     
    else
      if rain_fall_type == 'All'
        where('Union_Project = ? ', search).where('year = ?', year).order(:id)
      else
        if year == "All"
          where('Union_Project = ? ', search).order(:id)
        else
          where('Union_Project = ? ', search).where('year = ?', year).order(rain_fall_type)
        end
  ...
  def self.table(b, rain_fall_type, _year, ji, compare)
    ...
    if rain_fall_type 
      ...
      hash_data = ji.map do |el|
        if el.to_s == 'Union_Project'
          { title: 'Union Project', field: el, headerFilter: true }
        else
      ...
      if compare == 'None'
        hash_data = [
          { title: 'Union Project', field: 'Union_Project', headerFilter: true },
   ...
   if _year == "All"
    grouped = {}
    if rain_fall_type == "Daily_Milk_Collection"
        b.each do |x|
            grouped[x[:"Union_Project"]] ||= {}
            grouped[x[:"Union_Project"]][:"Union_Project"] = x[:"Union_Project"]
            grouped[x[:"Union_Project"]][x[:Year]] = x[:Daily_Milk_Collection]
          end
  ...
  def self.query(b, _year, rain_fall_type, views, ji, compare,search)
    ...
    if rain_fall_type == 'All'
            ...
            if search == "All"
              {
                ...
                legendText: dataset,
                showInLegend: true,
                dataPoints: b.reject { |x| x['Union_Project'] == 'Total' }.map do |el|
                  { y: el[column_name], z: el[column_name], label: el[d] }
                            end
              }
            else
      ...
      if compare != 'None'
        if _year == "All"
          grouped_data = b.group_by{ |data| data[:Union_Project]}
        ...
        hash_data =
          [{
            ...
            showInLegend: true,
            dataPoints: b.map do |el|
                          { y: el[rain_fall_type], label: el['Union_Project'] }
                        end
          }]
        end
          ...
          if search == "All"
            hash_data  = grouped_data.map{ |vegetable, values| 
              dataset = vegetable.to_s.gsub("_"," ")
              {
                ...
                name:dataset,
                showInLegend: true,
                dataPoints: values.reject { |x| x['Union_Project'] == 'Total' }.map { |value|
                  { y: value[rain_fall_type], label: value["Year"] }
                }
              }
           }
                 ...
                 hash_data.push(
                   {
                     ...
                     showInLegend: true,
                     dataPoints: [{ y: value[rain_fall_type], label:  value["Union_Project"] }]
                 }
                 )
               end
               }
     ...
     b.reject { |x| x['Union_Project'] == 'Total' }.map do |el|
                  {
                    type:views,
                    legendText:"#{el["Union_Project"]}",
                    showInLegend: true,
  ...
  def self.map1(b, rain_fall_type, _views, _ji, unit1, ranges)
      ...
      b.map.with_index do |el,i|
        ...
        if (0..6) === i
          ...
          hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
          # hash1 = { y: el[rain_fall_type], label: el["Union_Project"] }
          min.push(hash1)
        elsif (12..18) === i
          hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
          # hash1 = { y: el[rain_fall_type], label: el["Union_Project"] }
