class ResourceManagement4 < ApplicationRecord
  ...
  def self.search(search, compare, year, rain_fall_type)
    if search == 'All'
      ...
      if year == "All"
        where('Sources_of_Revenue = ? OR Sources_of_Revenue = ?', search, compare).order(:id)
      else
        where('Sources_of_Revenue = ? OR Sources_of_Revenue = ?', search, compare).where('year = ?', year).order(:id)
      end
      
    else
      if rain_fall_type == 'All'
        where('Sources_of_Revenue = ? ', search).where('year = ?', year).order(:id)
      else
        if year == "All"
          where('Sources_of_Revenue = ? ', search).order("id")
        else
          where('Sources_of_Revenue = ? ', search).where('year = ?', year).order(rain_fall_type)
        end
  ...
  def self.table(b, rain_fall_type, _year, ji, compare)
     ...
     if _year == "All"
      grouped = {}
        b.each do |x|
          grouped[x[:Sources_of_Revenue]] ||= {}
          grouped[x[:Sources_of_Revenue]][:Sources_of_Revenue] = x[:Sources_of_Revenue]
          grouped[x[:Sources_of_Revenue]][x[:Year]] = x[:Percentage_Growth_Over_Previous_Year]
        end
  ...
  def self.map_search(search, _compare, year, rain_fall_type)
    if search == 'All'
      ...
      # where(Sources_of_Revenue: search)
      where(Year: year).order(rain_fall_type)
    end
  end
    ...
    if rain_fall_type == 'All'
      ...
      if compare == "None"
        ...
        if _year == "All"
          ...
          if search == "All"
            h = b.reject{|x| x["Sources_of_Revenue"] == "Total"}.group_by{ |data| data[:Sources_of_Revenue]}
          ...
          if search == "All"
              h = b.reject{|x| x["Sources_of_Revenue"] == "Total"}
          else
              h = b
          end
          dataset = rain_fall_type.tr('_', ' ')
      ...
      if views == "stackedBar100" or views == "stackedBar"
        title = {
          ...
          if search.include? "percentage"
              l = "Percentage Growth Over Previous Year"
          else
              l = "Percentage Growth Over Previous Year"
  ...
  def self.map1(b, rain_fall_type, _views, _ji, unit1, ranges)
      ...
      b.map.with_index do |el,i|
        ...
        if (0..6) === i
          ...
          hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
          # hash1 = { y: el[rain_fall_type], label: el["Sources_of_Revenue"] }
          min.push(hash1)
        elsif (12..18) === i
          hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
          # hash1 = { y: el[rain_fall_type], label: el["Sources_of_Revenue"] }
