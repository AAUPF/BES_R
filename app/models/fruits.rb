module Fruits 
    ...
    def search(search,compare,year,rain_fall_type,fruits)
      if search == "All"
        # all.order(:WinterRain)
        #  abort(rain_fall_type)
          
          if rain_fall_type == "All"
            where(Year: year).where("fruit = ?", fruits).order('id Desc')

          else
            where(Year: year).where("fruit = ?", fruits).order("#{rain_fall_type} Desc")
          end
          
        # all
        # where(Districts: search)
      elsif compare == "Bihar vs District"
        where("Districts = ? OR Districts = ?", search, "Bihar").where("year = ?", year).where("fruit = ?", fruits).order("id")
      else
        
        if rain_fall_type == "All"
            where("Districts = ? ", search).where("year = ?", year).where("fruit = ?", fruits).order(:id)

        else
            where("Districts = ? ", search).where("year = ?", year).where("fruit = ?", fruits).order(rain_fall_type)
        end
    ...
    def map_search(search,compare,year,rain_fall_type,fruits)
      if search == "All"
    
        
        if rain_fall_type == "All"
          where(Year: year).where("fruit = ?", fruits).order(:id)
        else
          where(Year: year).where("fruit = ?", fruits).order(rain_fall_type)
        end
