class SixthEconomicCensus2 < ApplicationRecord
        ...
        def self.search(search, compare, _year, rain_fall_type)
          if search == 'All'
            ...
            where('District = ? OR District = ?', search, 'Bihar').order(:id)
          else
            if rain_fall_type == 'All'
              where('District = ? ', search).order(:id)
            else
              where('District = ? ', search).order(rain_fall_type)
            end
          end
        end
              ...
              hash_data = ji.map do |column_name|
                if compare == 'Bihar vs District'
                  ...
                  dataset = column_name.to_s.tr('_', ' ')
                  {
                    ...
                    legendText: dataset,
                    showInLegend: true,
                    dataPoints: b.reject { |x| x['District'] == 'Bihar' }.map do |el|
                                  { y: el[column_name], z: el[column_name], label: el[d] }
                                end
                  }
                end
            ...
            if compare == 'Bihar vs District'
              ...
              hash_data =
                [{
                  ...
                  showInLegend: true,
                  dataPoints: b.map do |el|
                                { y: el[rain_fall_type], label: el['District'] }
                              end
                }]
            else
              dataset = rain_fall_type.tr('_', ' ')
        ...
        def self.map(b, rain_fall_type, _views, _ji)
          ...
          b.map.with_index do |el,i|
            ...
            if (0..6) === i
              ...
              hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
              # hash1 = { y: el[rain_fall_type], label: el["District"] }
              min.push(hash1)
            elsif (12..18) === i
              hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
              # hash1 = { y: el[rain_fall_type], label: el["District"] }
