class ResourceManagement5 < ApplicationRecord
  ...
  def self.search(search, compare, year, rain_fall_type)
    if search == 'All'
      ...
      if year == "All"
        where('Source = ? OR Source = ?', search, compare).order(:id)
      else
        where('Source = ? OR Source = ?', search, compare).where('year = ?', year).order(:id)
      end
      
    else
      if rain_fall_type == 'All'
        where('Source = ? ', search).where('year = ?', year).order(:id)
      else
        if year == "All"
          where('Source = ? ', search).order("id")
        else
          where('Source = ? ', search).where('year = ?', year).order(rain_fall_type)
        end
  ...
  def self.table(b, rain_fall_type, _year, ji, compare)
      ...
      if rain_fall_type 
        ...
        hash_data = ji.map do |el|
          if el.to_s == 'Source'
            { title: 'Source', field: el, headerFilter: true }
          else
     ...
     if _year == "All"
      grouped = {}
        b.each do |x|
          grouped[x[:Source]] ||= {}
          grouped[x[:Source]][:Source] = x[:Source]
          grouped[x[:Source]][x[:Year]] = x[:Percentage]
        end
  ...
  def self.query(b, _year, rain_fall_type, views, ji, compare,search)
    ...
    if rain_fall_type == 'All'
      ...
      if compare == "None"
        ...
        if _year == "All"
          grouped_data = b.group_by{ |data| data[:Source]}
          if search == "All"
            h = b.reject{|x| x["Source"] == "Total "}.group_by{ |data| data[:Source]}
                    ...
                    hash_data =  b.map do |el|
                      {
                        ...
                        legendText:"#{el["Year"]}",
                        showInLegend: true,
                        dataPoints: [{ y: el[rain_fall_type], label:  el["Source"] }]
                      }
                    end
            end
            ...
            hash_data =
              [{
                ...
                dataPoints: h.map do |el|
                              { y: el[rain_fall_type], label: el['Source'] }
                            end
              }]
          else
            dataset = rain_fall_type.tr('_', ' ')
  ...
  def self.map1(b, rain_fall_type, _views, _ji, unit1, ranges)
      ...
      b.map.with_index do |el,i|
        ...
        if (0..6) === i
          ...
          hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
          # hash1 = { y: el[rain_fall_type], label: el["Source"] }
          min.push(hash1)
        elsif (12..18) === i
          hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
          # hash1 = { y: el[rain_fall_type], label: el["Source"] }
