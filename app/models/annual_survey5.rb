class AnnualSurvey5 < ApplicationRecord
    ...
    def self.search(search, compare, year, rain_fall_type)
      if search == 'All'
        ...
        if year == "All"
            where('Characteristics = ? OR Characteristics = ?', search, compare).order(:id)
        else
            where('Characteristics = ? OR Characteristics = ?', search, compare).where('year = ?', year).order(:id)
        end
      else
        
        if rain_fall_type == 'All'
            if year == "All"
                where('Characteristics = ? ', search).order(:id)
            else
                where('Characteristics = ? ', search).where('year = ?', year).order(:id)
            end
        else
            if year == "All"
                where('Characteristics = ? ', search).order(rain_fall_type)
            else
                where('Characteristics = ? ', search).where('year = ?', year).order(rain_fall_type)
            end
    ...
    def self.table(b, rain_fall_type, _year, ji, compare)
        ...
        if rain_fall_type 
          ...
          hash_data = ji.map do |el|
            if el.to_s == 'Characteristics'
              { title: 'Characteristics', field: el, headerFilter: true }
            else
          ...
          if compare == 'None'
            hash_data = [
              { title: 'Characteristics', field: 'Characteristics', headerFilter: true },
       ...
       if _year == "All"
        ...
        if rain_fall_type == "Bihar"
            b.each do |x|
                grouped[x[:Characteristics]] ||= {}
                grouped[x[:Characteristics]][:Characteristics] = x[:Characteristics]
                grouped[x[:Characteristics]][x[:Year]] = x[:Bihar]
              end
        else
            b.each do |x|
                grouped[x[:Characteristics]] ||= {}
                grouped[x[:Characteristics]][:Characteristics] = x[:Characteristics]
                grouped[x[:Characteristics]][x[:Year]] = x[:India]
              end
        end
        ...
        if compare == 'Bihar vs District'
          ...
          hash_data =
            [{
              ...
              showInLegend: true,
              dataPoints: b.map do |el|
                            { y: el[rain_fall_type], label: el['Characteristics'] }
                          end
            }]
        else
            if _year == "All"
                grouped_data = b.group_by{ |data| data[:Characteristics]}
                if search == "All"
                                  ...
                                  hash_data.push(
                                    {
                                      ...
                                      showInLegend: true,
                                      dataPoints: [{ y: value[rain_fall_type], label:  value["Characteristics"] }]
                                  }
                                  )
                    ...
                    hash_data = b.map do |col|
                      {
                        type:views,
                        toolTipContent: "{label}<br/>{name}, <strong>{y}</strong>",
                        name:col[:Characteristics],
                        legendText: col[:Characteristics],
                        showInLegend: true,
                ...
                hash_data =  b.map do |el|
                  {
                    type:views,
                    toolTipContent: "{label}<br/>{name}, <strong>{y}</strong>",
                    name:"#{el["Characteristics"]}",
                    legendText:"#{el["Characteristics"]}",
                    showInLegend: true,
