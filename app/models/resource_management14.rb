class ResourceManagement14 < ApplicationRecord
        ...
        def self.search(search, compare, _year, rain_fall_type)
          if search == 'All'
            ...
            if rain_fall_type == 'All'
              where('Agency = ? ', search).order(:id)
            else
              where('Agency = ? ', search).order(rain_fall_type)
            end
          end
        end
        ...
        def self.table(b, rain_fall_type, _year, ji, compare)
          ...
          if rain_fall_type == 'GoI_Scheme'
      
            hash_data = ji.map do |el|
              if el.to_s == 'Agency'
                { title: 'Agency', field: el, headerFilter: true }
              else
            ...
            if compare == 'None'
              hash_data = [
                { title: 'Agency', field: 'Agency', headerFilter: true },
