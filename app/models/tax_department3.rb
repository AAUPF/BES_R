class TaxDepartment3 < ApplicationRecord
    ...
    def self.search(search, compare, year, rain_fall_type)
      if search == 'All'
        ...
        if year == "All"
            where('Name_of_Commodity = ? OR Name_of_Commodity = ?', search, compare).order(:id)
        else
            where('Name_of_Commodity = ? OR Name_of_Commodity = ?', search, compare).where('year = ?', year).order(:id)
        end
      else
        
        if rain_fall_type == 'All'
            if year == "All"
                where('Name_of_Commodity = ? ', search).order(:id)
            else
                where('Name_of_Commodity = ? ', search).where('year = ?', year).order(:id)
            end
        else
            if year == "All"
                where('Name_of_Commodity = ? ', search).order(rain_fall_type)
            else
                where('Name_of_Commodity = ? ', search).where('year = ?', year).order(rain_fall_type)
            end
    ...
    def self.table(b, rain_fall_type, _year, ji, compare)
        ...
        if rain_fall_type 
          ...
          hash_data = ji.map do |el|
            if el.to_s == 'Name_of_Commodity'
              { title: 'Name of Commodity', field: el, headerFilter: true }
            else
          ...
          if compare == 'None'
            hash_data = [
              { title: 'Name of Commodity', field: 'Name_of_Commodity', headerFilter: true },
       ...
       if _year == "All"
        ...
        if rain_fall_type == "Collection"
            b.each do |x|
                grouped[x[:Name_of_Commodity]] ||= {}
                grouped[x[:Name_of_Commodity]][:Name_of_Commodity] = x[:Name_of_Commodity]
                grouped[x[:Name_of_Commodity]][x[:Year]] = x[:Collection]
              end
        else
            b.each do |x|
                grouped[x[:Name_of_Commodity]] ||= {}
                grouped[x[:Name_of_Commodity]][:Name_of_Commodity] = x[:Name_of_Commodity]
                grouped[x[:Name_of_Commodity]][x[:Year]] = x[:Rate_of_Growth]
              end
        end
    ...
    def self.map_search(search, _compare, year, rain_fall_type)
      if search == 'All'
        ...
        # where(Name_of_Commodity: search)
        where(Year: year).order(rain_fall_type)
      end
    end
    ...
    def self.map1(b, rain_fall_type, _views, _ji, unit1, ranges)
        ...
        b.map.with_index do |el,i|
          ...
          if (0..6) === i
            ...
            hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
            # hash1 = { y: el[rain_fall_type], label: el["Name_of_Commodity"] }
            min.push(hash1)
          elsif (12..18) === i
            hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
            # hash1 = { y: el[rain_fall_type], label: el["Name_of_Commodity"] }
