class StatePublicSector3 < ApplicationRecord
  ...
  def self.search(search,compare,year,rain_fall_type)
    ...
    if search == "All" and rain_fall_type == "All"
      all
        ...
        if compare == "None"
            if search == "All"
              select(rain_fall_type,"Sector")
            else
              select(rain_fall_type). where("Sector = ?", search)
            end
        else
            select(rain_fall_type,"Sector")
        end
         ...
         if compare == "None"
            ...
            select(rain_fall_type,"Sector").where("Sector = ?", search)
            end

         else

          if search == "All"
            select(rain_fall_type,compare,"Sector")
          else
            where("Sector = ?", search)
          end
...
def self.table (b,rain_fall_type,year,ji,compare)
    ...
    if compare == "None"
      ...
      hash_data = [{title:"Sector", field:"Sector",headerFilter:true},
        {title:dataset, field:rain_fall_type},
...
def self.query(b,year,rain_fall_type,views,ji,compare,search)
      ...
      if search == "All" && rain_fall_type == "All"
        ...
        hash_data =  ji.map do |column_name|
...

          {
            ...
            #name:dataset,
            showInLegend: true,
            dataPoints: b.reject{|x| x["Sector"]== "Total"}.map do |el|
       ...
       return title
        ...
        ji1 = [rain_fall_type,compare]
          ...
          dataset = rain_fall_type.tr('_', ' ')
                  hash_data =  b.reject{|x| x["Sector"]== "Total"}.map do |el|
                  {
                      type:views,
                      toolTipContent: "{label}<br/>{name}, <strong>{y}</strong>",
                      name:"#{el["Sector"]}",
     ...
     # logic of if end here
        ...
        if compare
          ...
          b.each do |element|
            hash1 = {:y => element[rain_fall_type] ,:label => element["Sector"]}
            array.push(hash1)
          end
