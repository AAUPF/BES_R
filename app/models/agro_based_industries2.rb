class AgroBasedIndustries2 < ApplicationRecord
  ...
  def self.search(search, compare, year, rain_fall_type)
    if search == 'All'
      ...
      if year == "All"
        where('Location_of_Union_or_Project = ? OR Location_of_Union_or_Project = ?', search, compare).order(:id)
      else
        where('Location_of_Union_or_Project = ? OR Location_of_Union_or_Project = ?', search, compare).where('year = ?', year).order(:id)
      end
     
    else
      if rain_fall_type == 'All'
        where('Location_of_Union_or_Project = ? ', search).where('year = ?', year).order(:id)
      else
        if year == "All"
          where('Location_of_Union_or_Project = ? ', search).order(:id)
        else
          where('Location_of_Union_or_Project = ? ', search).where('year = ?', year).order(rain_fall_type)
        end
  ...
  def self.table(b, rain_fall_type, _year, ji, compare)
    ...
    if rain_fall_type 
      ...
      hash_data = ji.map do |el|
        if el.to_s == 'Location_of_Union_or_Project'
          { title: 'Location of Union or Project', field: el, headerFilter: true }
        else
      ...
      if compare == 'None'
        hash_data = [
          { title: 'Location of Union or Project', field: 'Location_of_Union_or_Project', headerFilter: true },
   ...
   if _year == "All"
    ...
    if rain_fall_type == "Organised_Societies"
        b.each do |x|
            grouped[x[:Location_of_Union_or_Project]] ||= {}
            grouped[x[:Location_of_Union_or_Project]][:Location_of_Union_or_Project] = x[:Location_of_Union_or_Project]
            grouped[x[:Location_of_Union_or_Project]][x[:Year]] = x[:Organised_Societies]
          end
      elsif rain_fall_type == "Working_Societies"
        b.each do |x|
            grouped[x[:Location_of_Union_or_Project]] ||= {}
            grouped[x[:Location_of_Union_or_Project]][:Location_of_Union_or_Project] = x[:Location_of_Union_or_Project]
            grouped[x[:Location_of_Union_or_Project]][x[:Year]] = x[:Working_Societies]
          end
        elsif rain_fall_type == "Registered_Societies"
          b.each do |x|
              grouped[x[:Location_of_Union_or_Project]] ||= {}
              grouped[x[:Location_of_Union_or_Project]][:Location_of_Union_or_Project] = x[:Location_of_Union_or_Project]
              grouped[x[:Location_of_Union_or_Project]][x[:Year]] = x[:Registered_Societies]
            end
    end
  ...
  def self.map_search(search, _compare, year, rain_fall_type)
    if search == 'All'
      ...
      # where(Location_of_Union_or_Project: search)
      where(Year: year).order(rain_fall_type)
    end
  end

  def self.query(b, _year, rain_fall_type, views, ji, compare,search)
    d = 'Location_of_Union_or_Project'
    color  = "#4f81bc"
    if rain_fall_type == 'All'
      if views
            ...
            if search == "All"
              {
                ...
                legendText: dataset,
                showInLegend: true,
                dataPoints: b.reject { |x| x['Location_of_Union_or_Project'] == 'Total' }.map do |el|
                  { y: el[column_name], z: el[column_name], label: el[d] }
                            end
              }
            else
      ...
      if compare != 'None'
        if _year == "All"
          grouped_data = b.group_by{ |data| data[:Location_of_Union_or_Project]}
        ...
        hash_data =
          [{
            ...
            showInLegend: true,
            dataPoints: b.map do |el|
                          { y: el[rain_fall_type], label: el['Location_of_Union_or_Project'] }
                        end
          }]
        end
          ...
          if search == "All"
            hash_data  = grouped_data.map{ |vegetable, values| 
              dataset = vegetable.to_s.gsub("_"," ")
              {
                ...
                name:dataset,
                showInLegend: true,
                dataPoints: values.reject { |x| x['Location_of_Union_or_Project'] == 'Total' }.map { |value|
                  { y: value[rain_fall_type], label: value["Year"] }
                }
              }
           }
                 ...
                 hash_data.push(
                   {
                     ...
                     showInLegend: true,
                     dataPoints: [{ y: value[rain_fall_type], label:  value["Location_of_Union_or_Project"] }]
                 }
                 )
               end
               }
     ...
     b.reject { |x| x['Location_of_Union_or_Project'] == 'Total' }.map do |el|
                  {
                    type:views,
                    legendText:"#{el["Location_of_Union_or_Project"]}",
                    showInLegend: true,
  ...
  def self.map1(b, rain_fall_type, _views, _ji, unit1, ranges)
    ...
    b.map.with_index do |el, _i|
      dist = el['Location_of_Union_or_Project']
  ...
  def self.map1(b, rain_fall_type, _views, _ji, unit1, ranges)
      ...
      b.map.with_index do |el,i|
        ...
        if (0..6) === i
          ...
          hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
          # hash1 = { y: el[rain_fall_type], label: el["Location_of_Union_or_Project"] }
          min.push(hash1)
        elsif (12..18) === i
          hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
          # hash1 = { y: el[rain_fall_type], label: el["Location_of_Union_or_Project"] }
