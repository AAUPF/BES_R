class ResourceManagement13 < ApplicationRecord
  ...
  def self.search(search, compare, year, rain_fall_type)
    if search == 'All'
      ...
      if year == "All"
        where('Sources = ? OR Sources = ?', search, compare).order(:id)
      else
        where('Sources = ? OR Sources = ?', search, compare).where('year = ?', year).order(:id)
      end
      
    else
      if rain_fall_type == 'All'
        where('Sources = ? ', search).where('year = ?', year).order(:id)
      else
        if year == "All"
          where('Sources = ? ', search).order("id")
        else
          where('Sources = ? ', search).where('year = ?', year).order(rain_fall_type)
        end
  ...
  def self.table(b, rain_fall_type, _year, ji, compare)
      ...
      if rain_fall_type 
        ...
        hash_data = ji.map do |el|
          if el.to_s == 'Sources'
            { title: 'Sources', field: el, headerFilter: true }
          else
        ...
        if compare == 'None'
          hash_data = [
            { title: 'Sources', field: 'Sources', headerFilter: true },
     ...
     if _year == "All"
      grouped = {}
        b.each do |x|
          grouped[x[:Sources]] ||= {}
          grouped[x[:Sources]][:Sources] = x[:Sources]
          grouped[x[:Sources]][x[:Year]] = x[:Amount]
        end
  ...
  #       if _year == "All"
  #         grouped_data = b.group_by{ |data| data[:Sources]}
  #         if search == "All"
  #           h = b.reject{|x| x["Sources"] == "Total"}.group_by{ |data| data[:Sources]}
  ...
  #                         { y: el[rain_fall_type], label: el['Sources'] }
  #                       end
  #         }]
  #       end
                    ...
                    hash_data =  b.map do |el|
                      {
                        ...
                        legendText:"#{el["Year"]}",
                        showInLegend: true,
                        dataPoints: [{ y: el[rain_fall_type], label:  el["Sources"] }]
                      }
                    end
            end
  ...
  def self.map1(b, rain_fall_type, _views, _ji, unit1, ranges)
      ...
      b.map.with_index do |el,i|
        ...
        if (0..6) === i
          ...
          hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
          # hash1 = { y: el[rain_fall_type], label: el["Sources"] }
          min.push(hash1)
        elsif (12..18) === i
          hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
          # hash1 = { y: el[rain_fall_type], label: el["Sources"] }
