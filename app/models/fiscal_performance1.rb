     ...
     if _year == "All"
      ...
      if rain_fall_type == "Revenue_Deficit_GFD_Percentage"
          b.each do |x|
              ...
              grouped[x[:State]][x[:Year]] = x[:Revenue_Deficit_GFD_Percentage]
            end
          elsif rain_fall_type == "Capital_Outlay_GFD_Percentage"
            b.each do |x|
                grouped[x[:State]] ||= {}
                grouped[x[:State]][:State] = x[:State]
                grouped[x[:State]][x[:Year]] = x[:Capital_Outlay_GFD_Percentage]
              end
          elsif rain_fall_type == "Non_Dev_Exp_Agg_Disbursements_Percentage"
            b.each do |x|
                grouped[x[:State]] ||= {}
                grouped[x[:State]][:State] = x[:State]
                grouped[x[:State]][x[:Year]] = x[:Non_Dev_Exp_Agg_Disbursements_Percentage]
              end
          elsif rain_fall_type == "Non_Dev_Exp_Revenue_Receipts_Percentage"
            b.each do |x|
                grouped[x[:State]] ||= {}
                grouped[x[:State]][:State] = x[:State]
                grouped[x[:State]][x[:Year]] = x[:Non_Dev_Exp_Revenue_Receipts_Percentage]
              end
          elsif rain_fall_type == "Interest_Payments_Revenue_Exp_Percentage"
            b.each do |x|
                grouped[x[:State]] ||= {}
                grouped[x[:State]][:State] = x[:State]
                grouped[x[:State]][x[:Year]] = x[:Interest_Payments_Revenue_Exp_Percentage]
              end
          elsif rain_fall_type == "State_Own_Revenue_Revenue_Exp_Percentage"
            b.each do |x|
                grouped[x[:State]] ||= {}
                grouped[x[:State]][:State] = x[:State]
                grouped[x[:State]][x[:Year]] = x[:State_Own_Revenue_Revenue_Exp_Percentage]
              end
          elsif rain_fall_type == "Gross_Transfers_Aggregate_Disbursements_Percentage"
            b.each do |x|
                grouped[x[:State]] ||= {}
                grouped[x[:State]][:State] = x[:State]
                grouped[x[:State]][x[:Year]] = x[:Gross_Transfers_Aggregate_Disbursements_Percentage]
              end
          elsif rain_fall_type == "Debt_Servicing_Gross_Transfers_Percentage"
            b.each do |x|
                grouped[x[:State]] ||= {}
                grouped[x[:State]][:State] = x[:State]
                grouped[x[:State]][x[:Year]] = x[:Debt_Servicing_Gross_Transfers_Percentage]
              end
          else
      end
