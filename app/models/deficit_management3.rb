class DeficitManagement3 < ApplicationRecord
    ...
    def self.search(search, compare, year, rain_fall_type)
      if search == 'All'
        ...
        if year == "All"
          where('Gross_Fiscal_Deficit = ? OR Gross_Fiscal_Deficit = ?', search, compare).order(:id)
        else
          
          where('Gross_Fiscal_Deficit = ? OR Gross_Fiscal_Deficit = ?', search, compare).where('year = ?', year).order(:id)
        end
        
      else
        if rain_fall_type == 'All'
          where('Gross_Fiscal_Deficit = ? ', search).where('year = ?', year).order(:id)
        else
          if year == "All"
            where('Gross_Fiscal_Deficit = ? ', search).order("id")
          else
            where('Gross_Fiscal_Deficit = ? ', search).where('year = ?', year).order(rain_fall_type)
          end
    ...
    def self.table(b, rain_fall_type, _year, ji, compare)
      ...
      if rain_fall_type 
        ...
        hash_data = ji.map do |el|
          if el.to_s == 'Gross_Fiscal_Deficit'
            { title: 'Gross_Fiscal_Deficit', field: el, headerFilter: true }
          else
        ...
        if compare == 'None'
          hash_data = [
            { title: 'Gross_Fiscal_Deficit', field: 'Gross_Fiscal_Deficit', headerFilter: true },
     ...
     if _year == "All"
      grouped = {}
        b.each do |x|
          grouped[x[:Gross_Fiscal_Deficit]] ||= {}
          grouped[x[:Gross_Fiscal_Deficit]][:Gross_Fiscal_Deficit] = x[:Gross_Fiscal_Deficit]
          grouped[x[:Gross_Fiscal_Deficit]][x[:Year]] = x[:Amount]
        end
    ...
    def self.map_search(search, _compare, year, rain_fall_type)
      if search == 'All'
        ...
        # where(Gross_Fiscal_Deficit: search)
        where(Year: year).order(rain_fall_type)
      end
    end
    ...
    def self.map1(b, rain_fall_type, _views, _ji, unit1, ranges)
        ...
        b.map.with_index do |el,i|
          ...
          if (0..6) === i
            ...
            hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
            # hash1 = { y: el[rain_fall_type], label: el["Gross_Fiscal_Deficit"] }
            min.push(hash1)
          elsif (12..18) === i
            hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
            # hash1 = { y: el[rain_fall_type], label: el["Gross_Fiscal_Deficit"] }
