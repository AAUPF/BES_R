...
def self.table(b, rain_fall_type, _year, ji, compare)
     ...
     if _year == "All"
      grouped = {}
      if rain_fall_type == "Conventional_Deficit_Surplus"
        b.each do |x|
            grouped[x[:State]] ||= {}
            grouped[x[:State]][:State] = x[:State]
            grouped[x[:State]][x[:Year]] = x[:Conventional_Deficit_Surplus]
          end
        elsif rain_fall_type == "Revenue_Deficit_Surplus"
          b.each do |x|
              grouped[x[:State]] ||= {}
              grouped[x[:State]][:State] = x[:State]
              grouped[x[:State]][x[:Year]] = x[:Revenue_Deficit_Surplus]
            end
          else
          end
