class ResourceManagement11 < ApplicationRecord
  ...
  def self.search(search, compare, year, rain_fall_type)
    if search == 'All'
      ...
      if year == "All"
        where('Indicators = ? OR Indicators = ?', search, compare).order(:id)
      else
        where('Indicators = ? OR Indicators = ?', search, compare).where('year = ?', year).order(:id)
      end
      
    else
      if rain_fall_type == 'All'
        where('Indicators = ? ', search).where('year = ?', year).order(:id)
      else
        if year == "All"
          where('Indicators = ? ', search).order("id")
        else
          where('Indicators = ? ', search).where('year = ?', year).order(rain_fall_type)
        end
  ...
  def self.table(b, rain_fall_type, _year, ji, compare)
      ...
      if rain_fall_type 
        ...
        hash_data = ji.map do |el|
          if el.to_s == 'Indicators'
            { title: 'Indicators', field: el, headerFilter: true }
          else
        ...
        if compare == 'None'
          hash_data = [
            { title: 'Indicators', field: 'Indicators', headerFilter: true },
     ...
     if _year == "All"
      grouped = {}
        b.each do |x|
          grouped[x[:Indicators]] ||= {}
          grouped[x[:Indicators]][:Indicators] = x[:Indicators]
          grouped[x[:Indicators]][x[:Year]] = x[:Percentage]
        end
...
def self.query(b, _year, rain_fall_type, views, ji, compare,search)
    ...
    if rain_fall_type == 'All'
      ...
      if compare == "None"
        ...
        if _year == "All"
          grouped_data = b.group_by{ |data| data[:Indicators]}
          if search == "All"
            h = b.reject{|x| x["Indicators"] == "Total"}.group_by{ |data| data[:Indicators]}
                    ...
                    hash_data =  b.map do |el|
                      {
                        ...
                        legendText:"#{el["Year"]}",
                        showInLegend: true,
                        dataPoints: [{ y: el[rain_fall_type], label:  el["Indicators"] }]
                      }
                    end
            end
          ...
          if search == "All"
              h = b.reject{|x| x["Indicators"] == "Total"}
          else
              h = b
          end
          if views == "line" || views == "scatter" || views == "column"
              ...
              hash_data =
                [{
                  ...
                  dataPoints: h.map do |el|
                                { y: el[rain_fall_type], label: el['Indicators'] }
                              end
                }]
          else
            dataset = rain_fall_type.tr('_', ' ')
      ...
      if views == "stackedBar100" or views == "stackedBar"
        title = {
          ...
          if search == "All" && _year != "All" && views == "line" || views == "scatter" || views == "column"
            title = {
              animationEnabled: true,
              exportEnabled: true,
              title:{
  ...
  def self.map1(b, rain_fall_type, _views, _ji, unit1, ranges)
      ...
      b.map.with_index do |el,i|
        ...
        if (0..6) === i
          ...
          hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
          # hash1 = { y: el[rain_fall_type], label: el["Indicators"] }
          min.push(hash1)
        elsif (12..18) === i
          hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
          # hash1 = { y: el[rain_fall_type], label: el["Indicators"] }
