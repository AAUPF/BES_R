class AgroBasedIndustries1 < ApplicationRecord
        ...
        def self.search(search, compare, year, rain_fall_type)
          if search == 'All'
            ...
            if year == "All"
              where('Name_of_Sugar_Mill = ? OR Name_of_Sugar_Mill = ?', search, compare).order(:id)
            else
              where('Name_of_Sugar_Mill = ? OR Name_of_Sugar_Mill = ?', search, compare).where('year = ?', year).order(:id)
            end
           
          else
            if rain_fall_type == 'All'
              where('Name_of_Sugar_Mill = ? ', search).where('year = ?', year).order(:id)
            else
              if year == "All"
                where('Name_of_Sugar_Mill = ? ', search).order(:id)
              else
                where('Name_of_Sugar_Mill = ? ', search).where('year = ?', year).order(rain_fall_type)
              end
        ...
        def self.table(b, rain_fall_type, _year, ji, compare)
          ...
          if rain_fall_type 
            ...
            hash_data = ji.map do |el|
              if el.to_s == 'Name_of_Sugar_Mill'
                { title: 'Name of Sugar Mill', field: el, headerFilter: true }
              else
            ...
            if compare == 'None'
              hash_data = [
                { title: 'Name of Sugar Mill', field: 'Name_of_Sugar_Mill', headerFilter: true },
         ...
         if _year == "All"
          ...
          if rain_fall_type == "Sugarcane_Crushed"
              b.each do |x|
                  grouped[x[:Name_of_Sugar_Mill]] ||= {}
                  grouped[x[:Name_of_Sugar_Mill]][:Name_of_Sugar_Mill] = x[:Name_of_Sugar_Mill]
                  grouped[x[:Name_of_Sugar_Mill]][x[:Year]] = x[:Sugarcane_Crushed]
                end
            elsif rain_fall_type == "Sugar_Produced"
              b.each do |x|
                  grouped[x[:Name_of_Sugar_Mill]] ||= {}
                  grouped[x[:Name_of_Sugar_Mill]][:Name_of_Sugar_Mill] = x[:Name_of_Sugar_Mill]
                  grouped[x[:Name_of_Sugar_Mill]][x[:Year]] = x[:Sugar_Produced]
                end
              elsif rain_fall_type == "Recovery_Percentage"
                b.each do |x|
                    grouped[x[:Name_of_Sugar_Mill]] ||= {}
                    grouped[x[:Name_of_Sugar_Mill]][:Name_of_Sugar_Mill] = x[:Name_of_Sugar_Mill]
                    grouped[x[:Name_of_Sugar_Mill]][x[:Year]] = x[:Recovery_Percentage]
                  end
          end
        ...
        def self.query(b, _year, rain_fall_type, views, ji, compare,search)
          ...
          if rain_fall_type == 'All'
                  ...
                  if search == "All"
                    {
                      ...
                      legendText: dataset,
                      showInLegend: true,
                      dataPoints: b.reject { |x| x['Name_of_Sugar_Mill'] == 'Total' }.map do |el|
                        { y: el[column_name], z: el[column_name], label: el[d] }
                                  end
                    }
                  else
            ...
            if compare != 'None'
              if _year == "All"
                grouped_data = b.group_by{ |data| data[:Name_of_Sugar_Mill]}
              ...
              hash_data =
                [{
                  ...
                  showInLegend: true,
                  dataPoints: b.map do |el|
                                { y: el[rain_fall_type], label: el['Name_of_Sugar_Mill'] }
                              end
                }]
              end
                ...
                if search == "All"
                  hash_data  = grouped_data.map{ |vegetable, values| 
                    dataset = vegetable.to_s.gsub("_"," ")
                    {
                      ...
                      name:dataset,
                      showInLegend: true,
                      dataPoints: values.reject { |x| x['Name_of_Sugar_Mill'] == 'Total' }.map { |value|
                        { y: value[rain_fall_type], label: value["Year"] }
                      }
                    }
                 }
                       ...
                       hash_data.push(
                         {
                           ...
                           showInLegend: true,
                           dataPoints: [{ y: value[rain_fall_type], label:  value["Name_of_Sugar_Mill"] }]
                       }
                       )
                     end
                     }
           ...
           b.reject { |x| x['Name_of_Sugar_Mill'] == 'Total' }.map do |el|
                        {
                          type:views,
                          legendText:"#{el["Name_of_Sugar_Mill"]}",
                          showInLegend: true,
        ...
        def self.map1(b, rain_fall_type, _views, _ji, unit1, ranges)
            ...
            b.map.with_index do |el,i|
              ...
              if (0..6) === i
                ...
                hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
                # hash1 = { y: el[rain_fall_type], label: el["Name_of_Sugar_Mill"] }
                min.push(hash1)
              elsif (12..18) === i
                hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
                # hash1 = { y: el[rain_fall_type], label: el["Name_of_Sugar_Mill"] }
