class RegionalDisparity < ApplicationRecord
  ...
  def self.search(search, compare, _year, rain_fall_type)
    if search == 'All'
      ...
      where('Criteria = ? OR Criteria = ?', search, 'Bihar').order(:id)
    else
      if rain_fall_type == 'All'
        where('Criteria = ? ', search).order(:id)
      else
        where('Criteria = ? ', search).order(rain_fall_type)
      end
    end
  end
  ...
  def self.table(b, rain_fall_type, _year, ji, compare)
    ...
    if rain_fall_type == 'All'
      ...
      hash_data = ji.map do |el|
        if el.to_s == 'Criteria'
          { title: 'Criteria', field: el, headerFilter: true }
        else
      ...
      if compare == 'None'
        hash_data = [
          { title: 'Criteria', field: 'Criteria', headerFilter: true },
