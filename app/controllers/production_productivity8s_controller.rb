...
def test
  ...
  if rain_fall_type || views
    ...
    if views == "Map View"
      ...
      b = ProductionProductivity8.search(params[:search],compare,year,rain_fall_type,legend)
      a = ProductionProductivity8.table(b,rain_fall_type,year,ji1,compare,legend)
    else
      @ProductionProductivity8s = ProductionProductivity8.search(params[:search],compare,year,rain_fall_type,legend)
      a = ProductionProductivity8.query(@ProductionProductivity8s,params[:year],rain_fall_type,views,ji,compare,search,legend,remove)
    end
  ...
  private
    ...
    def production_productivity8_params
      params.require(:production_productivity8).permit(:Districts, :Area, :Production, :Productivity, :Year, :Percentage_Area, :Percentage_Production)
    end
end
