...
def test
  ...
  rain_fall_type = params[:rain_fall_type]
   ...
   if year == "All"
    ...
    if rain_fall_type != "All"
      ji1 = [:Sources, "#{rain_fall_type}", :Year]
  ...
  if rain_fall_type || views
    ...
    if views == "Map View"
      ...
      b = ResourceManagement13.search(params[:search],compare,year,rain_fall_type,legend)
      a = ResourceManagement13.table(b,rain_fall_type,year,ji1,compare,legend)
    else
      @ResourceManagement13 = ResourceManagement13.search(params[:search],compare,year,rain_fall_type,legend)
      a = ResourceManagement13.query(@ResourceManagement13,params[:year],rain_fall_type,views,ji,compare,search,legend,remove)
    end
    respond_to do |format|
      format.html { render json:a }
  end
