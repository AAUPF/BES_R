...
def test
  ...
  rain_fall_type = params[:rain_fall_type]
   ...
   if year == "All"
    ...
    if rain_fall_type != "All"
      ji1 = [:Revenue, "#{rain_fall_type}", :Year]
  ...
  if rain_fall_type || views
    ...
    if views == "Map View"
      ...
      b = RevenueAccount4.search(params[:search],compare,year,rain_fall_type,legend)
      a = RevenueAccount4.table(b,rain_fall_type,year,ji1,compare,legend)
    else
      @RevenueAccount4 = RevenueAccount4.search(params[:search],compare,year,rain_fall_type,legend)
      a = RevenueAccount4.query(@RevenueAccount4,params[:year],rain_fall_type,views,ji,compare,search,legend,remove)
    end
    respond_to do |format|
      format.html { render json:a }
  end
