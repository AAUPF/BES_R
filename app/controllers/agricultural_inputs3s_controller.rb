...
class AgriculturalInputs3sController < ApplicationController
  before_action :set_agricultural_inputs3, only: %i[show edit update destroy]
  ...
  def test
    ji = %i[Tractor Combine_Harvestors Zero_Tillage Pumpsets Power_Tiller
            Manually_operated_tools_implements Threshers]
    rain_fall_type = params[:rain_fall_type]
    views = params[:views]
    year = params[:year]
    compare = params[:compare]

    ji1 = %i[Year Tractor Combine_Harvestors Zero_Tillage Pumpsets Power_Tiller
    ...
    if rain_fall_type || views
      ...
      if views == 'Map View'
        if rain_fall_type == 'All'
          b = AgriculturalInputs3.map_search('All', compare, year,
                                             rain_fall_type)
          a = AgriculturalInputs3.map(b, params[:year], rain_fall_type, views)
        else
          b = AgriculturalInputs3.map_search(params[:search], compare,
                                             year, rain_fall_type)
          a = AgriculturalInputs3.map(b, rain_fall_type, year, ji)
        end
      elsif views == 'Table'
        b = AgriculturalInputs3.search(params[:search], compare, 
                                        year, rain_fall_type)
        a = AgriculturalInputs3.table(b, rain_fall_type, year, ji1, compare)
      else
        @AgriculturalInputs3s = AgriculturalInputs3.search(params[:search], compare,
                                                           year, rain_fall_type)
        a = AgriculturalInputs3.query(@AgriculturalInputs3s, params[:year],
  ...
  def create
    ...
    if @agricultural_inputs3.save
      redirect_to @agricultural_inputs3, notice: 'Agricultural inputs3 was '
    else
      render :new
    end
  end
  ...
  def destroy
    ...
    redirect_to agricultural_inputs3s_url, notice: 'Agricultural inputs3 was '
  end
