class SectoralExpenditure1sController < ApplicationController
  before_action :set_state_domestic_product5, only: %i[show edit update destroy]

  # GET /state_domestic_product5s
  def index
    @state_domestic_product5s = SectoralExpenditure1.all
  ...
  def new
    @state_domestic_product5 = SectoralExpenditure1.new
  end
    ...
    if rain_fall_type || views
      ...
      if views == 'Map View'
        ...
        if rain_fall_type == 'All'
          b = SectoralExpenditure1.map_search('All', compare, year, rain_fall_type)
          u = 'Total'
          a = SectoralExpenditure1.map(b, params[:year], rain_fall_type, views)
        else
          b = SectoralExpenditure1.map_search(params[:search], compare, year, rain_fall_type)
          a = SectoralExpenditure1.map(b, rain_fall_type, year, ji)
         end
      elsif views == 'Table'
        b = SectoralExpenditure1.search(params[:search], compare, year, rain_fall_type)
        a = SectoralExpenditure1.table(b, rain_fall_type, year, ji1, compare, search, data)
      else
        @SectoralExpenditure1s = SectoralExpenditure1.search(params[:search], compare, year, rain_fall_type)
        a = SectoralExpenditure1.query(@SectoralExpenditure1s, params[:year], rain_fall_type, views, ji, compare, search, data, jip)
      end
      respond_to do |format|
  ...
  def import
    ...
    SectoralExpenditure1.import1(params[:file])
  ...
  def create
    @state_domestic_product5 = SectoralExpenditure1.new(state_domestic_product5_params)
  ...
  def set_state_domestic_product5
    @state_domestic_product5 = SectoralExpenditure1.find(params[:id])
  end
