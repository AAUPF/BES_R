  ...
  def search(search, compare, year, rain_fall_type, legend)
    if search == 'All'
      if rain_fall_type == 'All'
        if year == 'All'
          where(Indicator: year).where('Indicator1 = ?', compare).order('id')
        else
          where(Indicator: year).where('Indicator1 = ?', compare).order(:id)
          end
      else
        if year == 'All'
          ...
          where(Indicator: year).where('Indicator1 = ?', compare).order("#{rain_fall_type} ")
        end
      end
    elsif compare != 'None'

      if year == 'All'
        where("#{legend} = ? OR #{legend} = ?", search, compare).where(Indicator: compare).order(:id)
      else
        where("#{legend} = ? OR #{legend} = ?", search, compare).where(Indicator: year).where('Indicator1 = ?', compare).order(:id)
      end
    else

      if rain_fall_type == 'All'
        if year == 'All'
          where("#{legend} = ? ", search).where(Indicator: year).where('Indicator1 = ?', compare).order(:id)
        else
          where("#{legend} = ? ", search).where(Indicator: year).where('Indicator1 = ?', compare).order(:id)
        end
      else
        if year == 'All'
          where("#{legend} = ? ", search).where(Indicator: year).where('Indicator1 = ?', compare).order(rain_fall_type)
        else
          where("#{legend} = ? ", search).where(Indicator: year).where('Indicator1 = ?', compare).order(rain_fall_type)
        end
    ...
    if rain_fall_type == 'All'
      ...
      if _year == 'All'
        ...
        if search == 'All'
          ...
          hash_data =  Newmodulefunctions.search_elese_all_two(b, views, legend, remove, rain_fall_type, color)
        end
        end
    end
