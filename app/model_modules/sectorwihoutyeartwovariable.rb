module Sectorwihoutyeartwovariable
    def import1(file)
    ...
    def search(search, compare, _year, rain_fall_type, legend)
      if search == 'All'
        ...
        where("#{legend} = ? OR #{legend} = ?", search, compare).where("Indicator = ? ", _year).order(:id)
      else
        if rain_fall_type == 'All'
          where("#{legend} = ? ", search).where("Indicator = ? ", _year).order(:id)
        else
          where("#{legend} = ? ", search).where("Indicator = ? ", _year).order(rain_fall_type)
        end
      end
    end
