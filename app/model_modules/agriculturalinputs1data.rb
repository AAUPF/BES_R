...
module Agriculturalinputs1data
  # extend Code
  def import1(file)
  ...
  def search(search, _compare, _year, _rain_fall_type)
    if search && _compare
        if search == "All"
          all
        elsif  search
            if _year == "All"
                if _compare == "All"
                    ...
                    where('Crops = ?', _compare)
                end
            else
                if _compare == "All"
                    where('Year = ?', _year)
                else
                    where('Crops = ?', _compare).where('Year = ?', _year)
                end
            end
        end
    elsif search == ""
  ...
  def query(b, year, rain_fall_type, views, ji, compare)
    ...
    if year == 'All' && compare == 'All'

        grouped_data = b.group_by{ |data| data[:Crops]}
          ...
          if compare == 'None'
            ...
            hash_data = 
              [{
                ...
                showInLegend: true,
                dataPoints: b.reject { |x| x['Districts'] == 'Bihar' }.map do |el|
                              { y: el[rain_fall_type], label: el['Crops'] }
                            end
              }]
