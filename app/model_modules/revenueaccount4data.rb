module Revenueaccount4data
    ...
    def search(search, compare, year, rain_fall_type)
      if search == 'All'
        ...
        if year == "All"
          where('Revenue = ? OR Revenue = ?', search, compare).order(:id)
        else
          where('Revenue = ? OR Revenue = ?', search, compare).where('year = ?', year).order(:id)
        end
        
      else
        if rain_fall_type == 'All'
          where('Revenue = ? ', search).where('year = ?', year).order(:id)
        else
          if year == "All"
            where('Revenue = ? ', search).order("id")
          else
            where('Revenue = ? ', search).where('year = ?', year).order(rain_fall_type)
          end
    ...
    def table(b, rain_fall_type, _year, ji, compare)
        ...
        if rain_fall_type 
          ...
          if compare == 'None'
            hash_data = [
              { title: 'Expenditure Pattern', field: 'Revenue', headerFilter: true },
       ...
       if _year == "All"
        grouped = {}
          b.each do |x|
            grouped[x[:Revenue]] ||= {}
            grouped[x[:Revenue]][:Revenue] = x[:Revenue]
            grouped[x[:Revenue]][x[:Year]] = x[:Amount]
          end
    ...
    #       if _year == "All"
    #         grouped_data = b.group_by{ |data| data[:Revenue]}
    #         if search == "All"
    #           h = b.reject{|x| x["Revenue"] == "Development Expenditure as percentage of Total Expenditure"}.group_by{ |data| data[:Revenue]}
    ...
    #       else
    #         if search == "All"
    #             h = b.reject{|x| x["Revenue"] == "Development Expenditure as percentage of Total Expenditure"}
    #         else
    #             h = b
    #         end
    #         dataset = rain_fall_type.tr('_', ' ')
    ...
    #                         { y: el[rain_fall_type], label: el['Revenue'] }
    #                       end
    #         }]
    #       end
                  ...
                  dataset = vegetable.to_s.gsub("_"," ")
                 {
                  ...
                  if views == "line" || views == "bubble" || views == "column"
                      ...
                      hash_data =  b.map do |el|
                      {
                          ...
                          legendText:"#{el["Year"]}",
                          showInLegend: true,
                          dataPoints: [{ y: el[rain_fall_type], label:el["Revenue"] }]
                      }
                          end
                  end
    ...
    def map1(b, rain_fall_type, _views, _ji, unit1, ranges)
        ...
        b.map.with_index do |el,i|
          ...
          if (0..6) === i
            ...
            hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
            # hash1 = { y: el[rain_fall_type], label: el["Revenue"] }
            min.push(hash1)
          elsif (12..18) === i
            hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
            # hash1 = { y: el[rain_fall_type], label: el["Revenue"] }
