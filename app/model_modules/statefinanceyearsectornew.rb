...
module Statefinanceyearsectornew
    def import1(file)
      ...
      def map_search(search, _compare, year, rain_fall_type,legend)
        if search == 'All'
          if rain_fall_type == 'All'
            where(Year: year).order(:id)
          else
        ...
        if rain_fall_type == 'All'
          ...
          if compare == "None"
            ...
            if _year == "All"
              ...
              if search == "All"
                h = b.reject{|x| x[legend] == "Total"}.group_by{ |data| data[legend]}
      ...
      def map1(b, rain_fall_type, _views, _ji, unit1, ranges)
          ...
          b.map.with_index do |el,i|
            ...
            if (0..6) === i
              ...
              hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
              # hash1 = { y: el[rain_fall_type], label: el[legend] }
              min.push(hash1)
            elsif (12..18) === i
              hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
              # hash1 = { y: el[rain_fall_type], label: el[legend] }
