...
module Statefinancethreevariable
  def import1(file)
  ...
  def search(search, compare, year, rain_fall_type)
    if search == 'All'
      ...
      where(Indicator1: compare)
      else
        if year == 'All'
          if rain_fall_type == "None"
            where('Sector = ? OR Sector = ? OR Sector = ?', "Primary", 'Secondary', 'Tertiary').where(Indicator: search).where(Indicator1: compare).order('id')
          else
            all
          ...
          if rain_fall_type == "All"
            ...
            where(Sector: rain_fall_type).where(Indicator1: compare).where(Indicator: search).order("#{year} ")
           end
        end
      ...
      if year == 'All'
        where('Sector = ? OR Sector = ?', search, 'Bihar').where(Indicator1: compare).where(Indicator: search).order(:id)
      else
        where('Sector = ? OR Sector = ?', search, 'Bihar').where(Indicator1: compare).where('year = ?', year).where(Indicator: search).order(:id)
      end
    # elsif compare != "None"
    # #   if year == 'All'
    ...
    # #   end
      ...
      if rain_fall_type == 'All'
        where(Indicator: search).where(Indicator1: compare).order('id')
      else
        if year == 'All'

          if rain_fall_type == "None"
            where('Sector = ? ', search).where(Indicator1: compare).where(Indicator: search).order('id')
          else

            where('Sector = ? ', rain_fall_type).where(Indicator1: compare).where(Indicator: search).order('id')
          end
        else
          if rain_fall_type == "None"
            if search == "All"
