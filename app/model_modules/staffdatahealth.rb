module Staffdatahealth
    ...
    def search(search,compare,year,rain_fall_type)
      if search == "All"
          ...
          where("Posts = ? OR Posts = ?", search,compare ).where("year = ?", year).order(:id)
      else
            if rain_fall_type == "All"
              where("Posts = ? ", search).where("year = ?", year).order(:id)
           else
              where("Posts = ? ", search).where("year = ?", year).order(rain_fall_type)
          end
      end
    end
      ...
      def table (b,rain_fall_type,year,ji,compare)
        ...
        if compare == "None"
          hash_data = [
            {title:"District", field:"Posts",headerFilter:true },
      ...
      if rain_fall_type == "All"
        ...
        if compare == "Bihar vs District"
          ...
          [{
            ...
            showInLegend: true,
            dataPoints: b.map do |el|
                 { y: el[rain_fall_type], label: el["Posts"] }
            end
          }]
        else
            dataset = rain_fall_type.gsub("_"," ")
    ...
    #  abort(rain_fall_type)
      ...
      b.map.with_index do |el,i|
        ...
        if (0..7) === i
          ...
          # hash1 = { y: el[rain_fall_type], label: el["Posts"] }
          min.push(hash1)
        elsif (15..21) === i
