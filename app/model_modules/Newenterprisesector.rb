      ...
      def search(search, compare, year, rain_fall_type)
        if search == 'All'
          ...
          if year == "All"
            ...
            where('Sector = ? OR Sector = ?', search, compare).where('year = ?', year).order(:id)
          end
          
        else
          if rain_fall_type == 'All'
            where('Sector = ? ', search).where('year = ?', year).order(:id)
          else
            if year == "All"
              where('Sector = ? ', search).order("id")
            else
              where('Sector = ? ', search).where('year = ?', year).order(rain_fall_type)
            end
      ...
      def table(b, rain_fall_type, _year, ji, compare)
         ...
         if _year == "All"
          grouped = {}
            b.each do |x|
              grouped[x[:Sector]] ||= {}
              grouped[x[:Sector]][:Sector] = x[:Sector]
              grouped[x[:Sector]][x[:Year]] = x[:Annual_Growth_Rate]
            end
      ...
      def query(b, _year, rain_fall_type, views, ji, compare,search)
        ...
        if rain_fall_type == 'All'
          ...
          if compare == "None"
            ...
            if _year == "All"
              grouped_data = b.group_by{ |data| data[:Sector]}
              if search == "All"
                h = b.group_by{ |data| data[:Sector]}
              ...
              hash_data =  b.map do |el|
                {
                  ...
                  legendText:"#{el["Year"]}",
                  showInLegend: true,
                  dataPoints: [{ y: el[rain_fall_type], label:  el["Sector"] }]
              }
