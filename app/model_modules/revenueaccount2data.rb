module Revenueaccount2data
    ...
    def search(search, compare, year, rain_fall_type)
      if search == 'All'
        ...
        if year == "All"
          ...
          where('Expenditure_Pattern = ? OR Expenditure_Pattern = ?', search, compare).where('year = ?', year).order(:id)
        end
        
      else
        if rain_fall_type == 'All'
          where('Expenditure_Pattern = ? ', search).where('year = ?', year).order(:id)
        else
          if year == "All"
            where('Expenditure_Pattern = ? ', search).order("id")
          else
            where('Expenditure_Pattern = ? ', search).where('year = ?', year).order(rain_fall_type)
          end
    ...
    def table(b, rain_fall_type, _year, ji, compare)
        ...
        if rain_fall_type 
          ...
          if compare == 'None'
            hash_data = [
              { title: 'Expenditure Pattern', field: 'Expenditure_Pattern', headerFilter: true },
              { title: dataset, field: rain_fall_type }
            ]
          else
       ...
       if _year == "All"
        grouped = {}
          b.each do |x|
            grouped[x[:Expenditure_Pattern]] ||= {}
            grouped[x[:Expenditure_Pattern]][:Expenditure_Pattern] = x[:Expenditure_Pattern]
            grouped[x[:Expenditure_Pattern]][x[:Year]] = x[:Amount]
          end
    ...
    def query(b, _year, rain_fall_type, views, ji, compare,search)
      ...
      if rain_fall_type == 'All'
        ...
        if compare == "None"
          ...
          if _year == "All"
            grouped_data = b.group_by{ |data| data[:Expenditure_Pattern]}
            if search == "All"
              h = b.reject{|x| x["Expenditure_Pattern"] == "Development Expenditure as percentage of Total Expenditure"}.group_by{ |data| data[:Expenditure_Pattern]}
            ...
            if search == "All"
                h = b.reject{|x| x["Expenditure_Pattern"] == "Development Expenditure as percentage of Total Expenditure"}
            else
                h = b
            end
            dataset = rain_fall_type.tr('_', ' ')
    ...
    def map1(b, rain_fall_type, _views, _ji, unit1, ranges)
        ...
        b.map.with_index do |el,i|
          ...
          if (0..6) === i
            ...
            hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
            # hash1 = { y: el[rain_fall_type], label: el["Expenditure_Pattern"] }
            min.push(hash1)
          elsif (12..18) === i
            hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
            # hash1 = { y: el[rain_fall_type], label: el["Expenditure_Pattern"] }
