      ...
      def search(search,compare,year,rain_fall_type)
          if year == "All" and rain_fall_type == "All"
            ...
            if compare == "None"
              # abort("error")
              if year == "All"
                select(rain_fall_type,"Year").where("Indicator = ?", search)
              else
                select(rain_fall_type). where("Year = ?", year).where("Indicator = ?", search)
              end
            else
              select(rain_fall_type,"Year").where("Indicator = ?", search)
            end
               ...
               if compare == "None"
                ...
                if rain_fall_type == "All"
                  where("Year = ?", year).where("Indicator = ?", search)
                else
                  # abort(rain_fall_type)
                  select(rain_fall_type,"Year").where("Year = ?", year).where("Indicator = ?", search)
                end
               else
                if year == "All"
                  select(rain_fall_type,compare,"Year").where("Indicator = ?", search)
                else
                  where("Year = ?", year).where("Indicator = ?", search)
                end
               end
