module Revenueaccount3data
    ...
    def search(search, compare, year, rain_fall_type)
      if search == 'All'
        ...
        if year == "All"
          where('Interest_Payment_and_Receipt = ? OR Interest_Payment_and_Receipt = ?', search, compare).order(:id)
        else
          where('Interest_Payment_and_Receipt = ? OR Interest_Payment_and_Receipt = ?', search, compare).where('year = ?', year).order(:id)
        end
        
      else
        if rain_fall_type == 'All'
          where('Interest_Payment_and_Receipt = ? ', search).where('year = ?', year).order(:id)
        else
          if year == "All"
            where('Interest_Payment_and_Receipt = ? ', search).order("id")
          else
            where('Interest_Payment_and_Receipt = ? ', search).where('year = ?', year).order(rain_fall_type)
          end
    ...
    def map_search(search, _compare, year, rain_fall_type)
      if search == 'All'
        ...
        # where(Interest_Payment_and_Receipt: search)
        where(Year: year).order(rain_fall_type)
      end
    end
    ...
    def query(b, _year, rain_fall_type, views, ji, compare,search)
      ...
      if rain_fall_type == 'All'
                ...
                dataset = vegetable.to_s.gsub("_"," ")
               {
                ...
                if views == "line" || views == "scatter" || views == "column"
                    ...
                    hash_data =  b.map do |el|
                    {
                        ...
                        legendText:"#{el["Year"]}",
                        showInLegend: true,
                        dataPoints: [{ y: el[rain_fall_type], label:el["Interest_Payment_and_Receipt"] }]
                    }
                        end
                end
        ...
        hash_data =  b.map do |el|
        {
            type:views,
            toolTipContent: "{label}<br/>{name}, <strong>{y}</strong>",
            name:"#{el["Interest_Payment_and_Receipt"]}",
            legendText:"#{el["Interest_Payment_and_Receipt"]}",
            showInLegend: true,
    ...
    def map1(b, rain_fall_type, _views, _ji, unit1, ranges)
      ...
      b.map.with_index do |el, _i|
        dist = el['Interest_Payment_and_Receipt']
    ...
    def map1(b, rain_fall_type, _views, _ji, unit1, ranges)
        ...
        b.map.with_index do |el,i|
          ...
          if (0..6) === i
            ...
            hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
            # hash1 = { y: el[rain_fall_type], label: el["Interest_Payment_and_Receipt"] }
            min.push(hash1)
          elsif (12..18) === i
            hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
            # hash1 = { y: el[rain_fall_type], label: el["Interest_Payment_and_Receipt"] }
