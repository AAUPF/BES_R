module Fundhealthdata
    ...
    def search(search,compare,year,rain_fall_type)
      if search == "All"
          ...
          where("NRHM = ? OR NRHM = ?", search,compare ).where("year = ?", year).order(:id)
      else
            if rain_fall_type == "All"
              where("NRHM = ? ", search).where("year = ?", year).order(:id)
           else
              where("NRHM = ? ", search).where("year = ?", year).order(rain_fall_type)
          end
      end
    end
      ...
      def table (b,rain_fall_type,year,ji,compare)
        ...
        if compare == "None"
          hash_data = [
            {title:"District", field:"NRHM",headerFilter:true },
      ...
      if rain_fall_type == "All"
        if views 
          hash_data =  ji.map do |column_name|
              ...
              dataset = column_name.to_s.gsub("_"," ")
            {
              ...
              legendText: dataset,
              showInLegend: true,
              dataPoints: b.reject{|x| x["NRHM"]== "Total"}.map do |el|
                { y: el[column_name],z:el[column_name], label: el[d] }
              end
            }
            end
        ...
        if compare == "Bihar vs District"
          ...
          [{
            ...
            showInLegend: true,
            dataPoints: b.map do |el|
                 { y: el[rain_fall_type], label: el["NRHM"] }
            end
          }]
        else
            dataset = rain_fall_type.gsub("_"," ")
    ...
    #  abort(rain_fall_type)
      ...
      b.map.with_index do |el,i|
        ...
        if (0..7) === i
          ...
          # hash1 = { y: el[rain_fall_type], label: el["NRHM"] }
          min.push(hash1)
        elsif (15..21) === i
