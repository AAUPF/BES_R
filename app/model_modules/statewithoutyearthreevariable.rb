module Statewithoutyearthreevariable
    def import1(file)
    ...
    def search(search, compare, _year, rain_fall_type,legend)
      if search == 'All'
        if rain_fall_type == 'All'
            where(Indicator: _year).where(Indicator1: compare).order('id ')
        else
            where(Indicator: _year).where(Indicator1: compare).order("#{rain_fall_type} ")
        end
      elsif compare == 'Bihar vs State'
        where("#{legend} = ? OR #{legend} = ?", search, 'Bihar').where(Indicator: _year).where(Indicator1: compare).order(:id)
      else
        if rain_fall_type == 'All'
          where("#{legend} = ? ", search).where(Indicator: _year).where(Indicator1: compare).order(:id)
        else
          where("#{legend} = ? ", search).where(Indicator: _year).where(Indicator1: compare).order(rain_fall_type)
        end
      end
    end
    ...
    def table(b, rain_fall_type, _year, ji, compare,legend)
      ...
      if rain_fall_type == 'All'
        ...
        if compare == 'None'
          hash_data = [
            { title: "#{legend}", field: "#{legend}", headerFilter: true },
          ...
          hash_data = ji.map do |column_name|
            if compare == 'Bihar vs State'
              ...
              dataset = column_name.to_s.tr('_', ' ')
              {
                ...
                legendText: dataset,
                showInLegend: true,
                dataPoints: b.reject { |x| x["#{legend}"] == 'India' }.map do |el|
                              { y: el[column_name], z: el[column_name], label: el[d] }
                            end
              }
            end
