  ...
  def search(search, compare, year, rain_fall_type)
    if year == "All" and rain_fall_type == "All"
      ...
      if compare == "None"
        # abort("error")
        if year == "All"
          select(rain_fall_type, "Year").where("Type_of_Fertilizer = ?", search)
        else
          select(rain_fall_type).where("Year = ?", year).where("Type_of_Fertilizer = ?", search)
        end
      else
        select(rain_fall_type, "Year").where("Type_of_Fertilizer = ?", search)
      end
      ...
      if compare == "None"
        ...
        if rain_fall_type == "All"
          where("Year = ?", year).where("Type_of_Fertilizer = ?", search)
        else
          # abort(rain_fall_type)
          select(rain_fall_type, "Year").where("Year = ?", year).where("Type_of_Fertilizer = ?", search)
        end
      else
        if year == "All"
          select(rain_fall_type, compare, "Year").where("Type_of_Fertilizer = ?", search)
        else
          where("Type_of_Fertilizer = ?", search).where("Year = ?", year)
        end
      end
    end
  end
