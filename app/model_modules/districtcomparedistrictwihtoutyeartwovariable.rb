module Districtcomparedistrictwihtoutyeartwovariable
      ...
      def search(search, compare, _year, rain_fall_type,legend)
        if search == 'All'
          if rain_fall_type == 'All'
            where("#{legend} = ? ", _year).order('id ')
          else
            where("#{legend} = ? ", _year).order("#{rain_fall_type} ")
          end
        elsif compare != 'None'
          where('Districts = ? OR Districts = ?', search, compare).where("#{legend} = ? ", _year).order(:id)
        else
          if rain_fall_type == 'All'
            where('Districts = ? ', search).where("#{legend} = ? ", _year).order(:id)
          else
            where('Districts = ? ', search).where("#{legend} = ? ", _year).order(rain_fall_type)
          end
        end
      end
