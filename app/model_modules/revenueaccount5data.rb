module Revenueaccount5data
    ...
    def search(search, compare, year, rain_fall_type)
      if search == 'All'
        ...
        if year == "All"
          where('Transfer_of_Resources = ? OR Transfer_of_Resources = ?', search, compare).order(:id)
        else
          where('Transfer_of_Resources = ? OR Transfer_of_Resources = ?', search, compare).where('year = ?', year).order(:id)
        end
        
      else
        if rain_fall_type == 'All'
          where('Transfer_of_Resources = ? ', search).where('year = ?', year).order(:id)
        else
          if year == "All"
            where('Transfer_of_Resources = ? ', search).order("id")
          else
            where('Transfer_of_Resources = ? ', search).where('year = ?', year).order(rain_fall_type)
          end
    ...
    def table(b, rain_fall_type, _year, ji, compare)
      ...
      if rain_fall_type == 'All'
        ...
        hash_data = ji.map do |el|
          if el.to_s == 'Transfer_of_Resources'
            { title: 'Transfer_of_Resources', field: el, headerFilter: true }
          else
        ...
        if compare == 'None'
          hash_data = [
            { title: 'Transfer_of_Resources', field: 'Transfer_of_Resources', headerFilter: true },
      ...
      #     j.push({id: k.id, Productivity: u ,Transfer_of_Resources: k.Transfer_of_Resources, Area: k.Area, Production: k.Production, Year: k.Year })
      #   else
      #     j.push(k)
      #   end
      # end
    ...
    def map_search(search, _compare, year, rain_fall_type)
      if search == 'All'
        ...
        # where(Transfer_of_Resources: search)
        where(Year: year).order(rain_fall_type)
      end
    end
      ...
      if rain_fall_type == 'All'
        ...
        if compare
          if _year == "All"
            ...
            { y: value[rain_fall_type], label: value[:Transfer_of_Resources] }
            }
            }
            }
          else
    ...
    def map1(b, rain_fall_type, _views, _ji, unit1, ranges)
        ...
        b.map.with_index do |el,i|
          ...
          if (0..6) === i
            ...
            hash1 = { y: el[rain_fall_type], label: dist, color: "Orange" }
            # hash1 = { y: el[rain_fall_type], label: el["Transfer_of_Resources"] }
            min.push(hash1)
          elsif (12..18) === i
            hash1 = { y: el[rain_fall_type], label: dist, color: "Dark_Yellow" }
            # hash1 = { y: el[rain_fall_type], label: el["Transfer_of_Resources"] }
