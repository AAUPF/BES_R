module Test
  ...
  def map(b, rain_fall_type, _views, _ji, unit1)
    ...
    if below_min
      b = { min: below_min.first[:y].round(-below_min.first[:y].round.to_s.length + 1), max: "#{below_min.last[:y].round(-below_min.first[:y].round.to_s.length + 1)} #{unit1}" }
    else
      b = {}
    end

    if min.any?
      c = { min: min.first[:y].round(-below_min.first[:y].round.to_s.length + 1), max: "#{min.last[:y].round(-below_min.first[:y].round.to_s.length + 1)} #{unit1}" }
    else
      c = {}
    end

    if blow_max.any?
      d = { min: blow_max.first[:y].round(-below_min.first[:y].round.to_s.length + 1), max: "#{blow_max.last[:y].round(-below_min.first[:y].round.to_s.length + 1)} #{unit1}" }
    else
      d = {}
    end

    if max.any?
      e = { min: max.first[:y].round(-below_min.first[:y].round.to_s.length + 1), max: "#{max.last[:y].round(-below_min.first[:y].round.to_s.length + 1)} #{unit1}" }
    else
      e = {}
    end
    ...
    if above_max.any?
      ...
      f = { min: above_max.first[:y].round(-below_min.first[:y].round.to_s.length + 1), max: "#{above_max.last[:y].round(-below_min.first[:y].round.to_s.length + 1)} #{unit1}" }
    ...
    if extreme.any?
      g = { min: extreme.first[:y].round(-below_min.first[:y].round.to_s.length + 1), max: "#{extreme.last[:y].round(-below_min.first[:y].round.to_s.length + 1)} #{unit1}" }
    ...
    if above_extreme.any?
      h = { max: "#{above_extreme.first[:y].round(-below_min.first[:y].round.to_s.length + 1)} #{unit1}" }
    else
      h = {}
    end
    hu = {
